...
RUN set -x && mkdir -pv ~/tmp && cd ~/tmp \
  ...
  && cmake -D OPENCV_EXTRA_MODULES_PATH=$CONTRIB -D BUILD_opencv_contrib_world=ON -D WITH_OPENCL=OFF -D WITH_IPP=OFF -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/var/task ../ \
  && make install
