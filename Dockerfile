...
RUN set -x && mkdir -pv ~/tmp && cd ~/tmp \
  ...
  && PKG_CONFIG_PATH=/var/task/lib/pkgconfig cmake -D OPENCV_EXTRA_MODULES_PATH=$CONTRIB -D CMAKE_BUILD_TYPE=RELEASE -D BUILD_opencv_python2=ON -D CMAKE_INSTALL_PREFIX=/var/task ../ \
